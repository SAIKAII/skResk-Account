...
import (
	...
	_ "github.com/SAIKAII/skResk-Account/testx"
	...
	err := base.Tx(func(runner *dbx.TxRunner) error {
		...
		Convey("更新账户余额", t, func() {
			...
			Convey("扣减余额，余额足够", func() {
				...
				rows, err := dao.UpdateBalance(a1.AccountNo, amount)
				So(err, ShouldBeNil)
				So(rows, ShouldEqual, 1)
				a2 := dao.GetOne(a.AccountNo)
				newBalance := balance.Add(amount)
				So(a2, ShouldNotBeNil)
				So(a2.Balance.String(), ShouldEqual, newBalance.String())
			})
