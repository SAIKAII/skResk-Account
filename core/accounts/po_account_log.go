...
type AccountLog struct {
	...
	AccountNo       string              `db:"account_no"`           // 账户编号 发起者账户ID
	UserId          string              `db:"user_id"`              // 用户编号
	Username        string              `db:"username"`             // 用户名称
	TargetAccountNo string              `db:"target_account_no"`    // 账户编号 目标用户账户ID
	...
	ChangeType      services.ChangeType `db:"change_type"`          // 流水交易类型，0创建账户 >0为收入类型 <0为支出类型，自定义
	ChangeFlag      services.ChangeFlag `db:"change_flag"`          // 交易变化标识：-1出账 1入账 枚举
	Status          int                 `db:"status"`               // 交易状态
	Desc            string              `db:"desc"`                 // 交易描述
